...
from explicits import UserExplicits
# Testing
import pytest
...
def test_newline(tmpdir):
    ...
    tmp_path.write("\n".join(correct_arr))
    # Test - create a UserExplicits and see whether or not it loads the data
    fl = UserExplicits(str(tmp_path))
...
def test_csv_file(tmpdir):
    ...
    # Test - create a UserExplicits and see whether or not it loads the data
    fl = UserExplicits(str(tmp_path), separator)
    # The array which we read from the file should be the same as the array which we wrote
    assert fl.set == correct_arr
