...
class AudioFile:
    ...
    def __create_chunks(
        ...
        offset = 2500 if overlapping else 0
        chunks = audio_segment[offset::self.chunk_length]
        for index, chunk in enumerate(chunks):
            chunk_path = self.__create_chunk(
                index, chunk, 'wav', temp_dir, overlapping, offset)
        ...
        self.__last_overlapping_chunk(
            audio_segment, temp_dir, chunks_arr, overlapping, offset)

    def __create_chunk(
            self, index, chunk, ext, temp_dir, overlapping, offset, faux=False):
        ...
        with open(accuracy_path, 'wb') as chunk_file:
            self.__accuracy_chunk(
                chunk, accuracy_path, index, offset, chunk_file, ext, faux)
        ...
        with open(file_path, 'wb') as chunk_file:
            chunk.export(chunk_file, format=ext)
            return chunk_file.name

    def __last_overlapping_chunk(
            self, audio_segment, temp_dir, chunks_arr, overlapping, offset):
        ...
        if overlapping and len(audio_segment) % self.chunk_length < 4000:
            chunk_path = self.__create_chunk(
                ...
                'wav', temp_dir, overlapping, offset, True)
            chunks_arr.append(chunk_path)

    def __accuracy_chunk(
            self, chunk, path, index, offset, chunk_file, ext, faux):
        if faux:
            chunk.export(chunk_file, format=ext)
        else:
            # Write the file, apply accuracy measures and write again
            chunk.export(chunk_file, format=ext)
            accuracy_chunk = improve_accuracy(
                chunk, path, self.chunk_length, index, offset)
            accuracy_chunk.export(chunk_file, format=ext)
